...
import numpy as np
import pandas as pd
baseline_rate = 5.0  # events per experiment
treatment_effect = {
    "Saline": 1.0,
    "Pregabalin": 1.1,
    "Compound 1": 1.3,
...
cage_injury_effect = {
    ("Probe", "Yes"): 0.6,   # strong suppression in injured animals
    ("Probe", "No"): 1.0     # minimal or no effect in uninjured animals
}
...
"""Event counts were analysed using a generalized linear model with a log link, including treatment, injury state, probe condition and all interaction terms. Results are reported as rate ratios relative to the reference condition: Saline-treated, uninjured animals in the no-probe condition.

The predicted reference count was 10.08. Relative to Saline, all treatments increased event rates, with Morphine producing the largest increase (RR = 1.48), followed by Compound 1 (RR = 1.30) and Pregabalin (RR = 1.11). Injury reduced counts to 48% of the uninjured Saline rate (RR = 0.48), while probe exposure reduced counts to 64% of the no-probe Saline rate (RR = 0.64).

Treatment modified the injury effect. Under no-probe conditions, Morphine reversed injury-associated suppression (combined injury effect: RR = 1.32), while Compound 1 produced partial rescue (RR = 0.80) and Pregabalin produced a weaker rescue effect (RR = 0.57). Saline-treated injured animals showed the greatest reduction in event counts (RR = 0.48). Probe exposure further suppressed counts across groups, particularly in injured Saline-treated animals, which were predicted to retain only 18% of the reference event rate.

Under injury + probe conditions, Morphine produced the highest predicted event rate (8.33 counts), followed by Compound 1 (4.17 counts), Pregabalin (3.00 counts) and Saline (1.83 counts). Overall, the model suggests that injury and probe exposure suppress event counts, while Morphine most strongly restores behaviour, with Compound 1 showing an intermediate effect and Pregabalin a weaker effect.
...
for idx, trt in enumerate(treatments_present):
    ...
    if idx in [0, 2]:
        ax.set_ylabel("Chamber 2 crossings", fontsize=21, weight="bold")
    else:
        ax.set_ylabel("")
...
plt.savefig("synthetic_chamber2_crossing_bar_2x2.png", dpi=600, bbox_inches="tight")
plt.savefig("synthetic_chamber2_crossing_bar_2x2.pdf", dpi=600, bbox_inches="tight")
plt.show()
