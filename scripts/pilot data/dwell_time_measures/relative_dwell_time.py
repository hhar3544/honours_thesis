"""
Graph-only dwell time analysis for MCA ROI data.

This version:
- reads Roi.csv from SimBA ROI outputs
- calculates chamber dwell time metrics
- makes the dwell time figure only
- saves only the graph outputs (PNG and PDF)
...
PROBE_ORDER = [0, 5]
CHAMBER_ORDER = ["Chamber1", "Chamber2", "Chamber3"]
...
def load_and_process_data(input_csv):
    ...
    time_df = raw[
        (raw["MEASUREMENT"].astype(str) == "TOTAL ROI TIME (S)") &
        (raw["SHAPE"].astype(str).isin(CHAMBER_ORDER))
    ].copy()

    if time_df.empty:
        raise ValueError("No TOTAL ROI TIME (S) rows found for Chamber1, Chamber2 or Chamber3.")

    parsed = time_df["VIDEO"].apply(parse_video_name)
    time_df = pd.concat([time_df, parsed], axis=1)

    time_df = time_df[time_df["Treatment"].isin(TREATMENT_ORDER)].copy()
    time_df = time_df[time_df["Injury"].isin(INJURY_ORDER)].copy()
    time_df = time_df[time_df["Probe"].isin(PROBE_ORDER)].copy()

    if "BODY-PART" in time_df.columns:
        bodyparts = set(time_df["BODY-PART"].astype(str).str.lower().unique())
        if "nose" in bodyparts:
            time_df = time_df[time_df["BODY-PART"].astype(str).str.lower() == "nose"].copy()
        else:
            time_df = (
                time_df.sort_values(["VIDEO", "SHAPE", "BODY-PART"])
                .drop_duplicates(subset=["VIDEO", "SHAPE"], keep="first")
                .copy()
            )

    time_wide = (
        time_df.pivot_table(
            index=["VIDEO", "Treatment", "Injury", "AnimalID", "AnimalUID", "Probe"],
            columns="SHAPE",
            values="VALUE",
            aggfunc="sum",
            fill_value=0,
    ...
    for chamber in CHAMBER_ORDER:
        if chamber not in time_wide.columns:
            time_wide[chamber] = 0.0

    df = time_wide.rename(columns={
        "Chamber1": "time_ch1_s",
        "Chamber2": "time_ch2_s",
        "Chamber3": "time_ch3_s",
    })

    df["total_time_chambers_s"] = df["time_ch1_s"] + df["time_ch2_s"] + df["time_ch3_s"]

    df["pct_time_ch1"] = np.where(
        df["total_time_chambers_s"] > 0,
        (df["time_ch1_s"] / df["total_time_chambers_s"]) * 100,
        np.nan,
    )

    df["pct_time_ch2"] = np.where(
        df["total_time_chambers_s"] > 0,
        (df["time_ch2_s"] / df["total_time_chambers_s"]) * 100,
        np.nan,
    )

    df["pct_time_ch3"] = np.where(
        df["total_time_chambers_s"] > 0,
        (df["time_ch3_s"] / df["total_time_chambers_s"]) * 100,
        np.nan,
    )
...
def make_figure(df, output_dir):
    make_metric_plot(
        df=df,
        output_dir=output_dir,
        outcome_col="pct_time_ch2",
        y_label="Relative time in Chamber 2 (%)",
        output_prefix="dwell_pct_time_ch2_2x2",
    )

    make_metric_plot(
        df=df,
        output_dir=output_dir,
        outcome_col="time_ch2_s",
        y_label="Total time in Chamber 2 (s)",
        output_prefix="dwell_total_time_ch2_2x2",
    )
