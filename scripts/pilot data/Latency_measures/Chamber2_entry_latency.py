"""
Graph-only Chamber 2 entry latency analysis.

This version:
- reads Roi.csv from SimBA ROI outputs
- extracts Chamber 2 entry latency = FIRST ROI ENTRY TIME (S) into Chamber2
- makes the figure only
...
TARGET_MEASUREMENT = "FIRST ROI ENTRY TIME (S)"
OUTCOME_COL = "VALUE"
...
def load_and_process_data(input_csv):
    ...
    df = raw[
        (raw["MEASUREMENT"].astype(str) == TARGET_MEASUREMENT) &
        (raw["SHAPE"].astype(str) == TARGET_SHAPE)
    ].copy()

    if df.empty:
        raise ValueError(
            f"No rows found for MEASUREMENT == {TARGET_MEASUREMENT!r} and SHAPE == {TARGET_SHAPE!r}."
        )
...
def make_plot(df, output_dir):
    ...
    for col_idx, treatment in enumerate(treatments_present):
        ...
        if col_idx in [0, 2]:
            ax.set_ylabel("Entry latency to Chamber 2 (s)", fontsize=21, weight="bold")
        else:
            ax.set_ylabel("")
    ...
    png_path = output_dir / "chamber2_entry_latency_large_2x2.png"
    pdf_path = output_dir / "chamber2_entry_latency_large_2x2.pdf"
